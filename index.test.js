...
describe('[Exercise 3] findLargestInteger', () => {
  test('[5] returns the largest number in an array of objects { integer: 2 }', () => {
    ...
    const input2 = [{ integer: 5 }, { integer: 4 }, { integer: 3 }]
    const input3 = [{ integer: 3 }, { integer: 4 }, { integer: 1 }]
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  test('[19] resolves true if passed an even number', async () => {
        const evens = await utils.isEvenNumberAsync(12);
        expect(evens).toBe(true)
  })
  test('[20] resolves false if passed an odd number', async () => {
    const odds = await utils.isEvenNumberAsync(39);
    expect(odds).toBe(false)
  })
})
